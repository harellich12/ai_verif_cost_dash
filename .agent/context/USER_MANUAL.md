...
### Admin vs Presentation vs Sales (global)
...
- **Header toggles**:
  - `ROI / VaaS`
  - `Admin / Presentation / Sales (VaaS only)`
- **Presentation Setup**:
  - Select which sections are visible in Presentation mode and Sales mode.
...
- **VaaS workbook**:
  - quote summary
  - schedule sheet
- **VaaS Sales workbook** (when `Sales` view is active):
  - external-cost summary
  - traditional/external schedule
  - excludes VaaS quote/client-review/net-benefit rows
- All exports are **snapshot/value-only** (no live formulas).

## 7. Presentation / Sales Workflow
1. Stay in **Admin** mode.
2. Open **Presentation Setup**.
3. Enable only the sections you want clients to see.
4. Click **Preview as Client**.
5. Present in **Presentation** mode or switch to **Sales** mode in VaaS.
...
- If exports differ from expectation, confirm active app/view mode (`ROI` vs `VaaS`, and `Admin` vs `Presentation` vs `Sales`) and current inputs.
